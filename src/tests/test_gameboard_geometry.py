...
def test_neighbors_center(board_3x3):
    """
    Center cell should have 4 neighbors.
    """
    neighbors = set(board_3x3.neighbors(1, 1))
    assert neighbors == {(0, 1), (2, 1), (1, 0), (1, 2)}


def test_neighbors_corner(board_3x3):
    """
    Corner cell should have 2 neighbors.
    """
    neighbors = set(board_3x3.neighbors(0, 0))
    assert neighbors == {(1, 0), (0, 1)}


def test_neighbors_edge(board_3x3):
    """
    Edge (non-corner) cell should have 3 neighbors.
    """
    neighbors = set(board_3x3.neighbors(1, 0))
    assert neighbors == {(0, 0), (2, 0), (1, 1)}


def test_neighbors_all_within_bounds(board_3x3):
    """
    neighbors must never return out-of-bounds coordinates.
    """
    for x in range(3):
        for y in range(3):
            for nx, ny in board_3x3.neighbors(x, y):
                assert board_3x3.is_in_bounds(nx, ny)


def test_neighbors_no_diagonals(board_3x3):
    """
    neighbors must not include diagonal neighbors if no parameter is passed.
    """
    neighbors = set(board_3x3.neighbors(1, 1))
...
def test_neighbors_with_diagonals(board_3x3):
    """
    all neighbors must be include if optional parameter "8" is passed.
    """
    neighbors = set(board_3x3.neighbors(1, 1, 8))
